...
test('redactEvent: request の結果の setCookies(個別 Set-Cookie)が既定でマスク、--no-mask で残る', () => {
  const result = JSON.stringify({
    status: 200,
    setCookies: ['sid=abc; Path=/; HttpOnly', 'csrf=xyz; Path=/'],
    body: 'ok',
  });
  const masked = JSON.parse((redactEvent(cmdEvent('request', { url: 'https://a/b' }, { result }), M) as CommandEvent).result!);
  assert.deepEqual(masked.setCookies, [MASK, MASK]);
  const kept = JSON.parse((redactEvent(cmdEvent('request', { url: 'https://a/b' }, { result }), NO_M) as CommandEvent).result!);
  assert.deepEqual(kept.setCookies, ['sid=abc; Path=/; HttpOnly', 'csrf=xyz; Path=/']);
});
