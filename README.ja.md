...
> **想定している 2 つの使い方:**
> - **AI エージェントから操作** — Claude Code などのエージェントが **MCP(23 ツール)** または **Bash**(全コマンド `--json` 対応)で kb を駆動。アクセシビリティ snapshot + `--ref` のループは低トークンで確実な自動操作に最適化され、常駐デーモンにより各操作は数十 ms で返る。
...
- **MCP サーバ** — `kb-mcp` が 23 ツールを公開(スクリーンショットは画像で返る)
...
| 分類 | コマンド |
|---|---|
| デーモン | `kb daemon start [--headless] [--profile <n>] [--channel chrome\|msedge\|chromium] [--ua <s>] [--stealth] [--cdp <url>] / stop / status` |
...
`kb_snapshot` `kb_open` `kb_text` `kb_screenshot`(画像で返る)`kb_click` `kb_fill` `kb_select` `kb_eval` `kb_request` `kb_net_log` `kb_net_body` `kb_net_headers` `kb_proxy_use` など 23 ツールを公開。
