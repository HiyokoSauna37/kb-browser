...
- **本物の Chrome** — インストール済み Chrome/Edge を使用(DRM も有効)。なければ同梱 Chromium にフォールバック。`--channel` で明示選択、`--ua` で User-Agent 上書きも可
...
| 分類 | コマンド |
|---|---|
| デーモン | `kb daemon start [--headless] [--profile <n>] [--channel chrome\|msedge\|chromium] [--ua <s>] [--cdp <url>] / stop / status` |
